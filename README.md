...
`cl.exe` is the reason for much internal complexity. Obtaining
dependency information from `cl.exe` requires a vastly different
approach to that used with the gcc-style compilers, which just require
some extra options to do what we need. `cl.exe` needs an option,
`/showIncludes`, and have the resultant output _scraped_ to obtain the
names of the included, dependent, files.

### `#include` in options files

The inheritence mechanism

### Improve `.dcc` directory support
